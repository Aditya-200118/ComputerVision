...
\begin{shaded}
\textbf{\textit{Note on Difference Maps:}} In the error analysis heatmaps (Figures 4 and 10), a \textbf{completely black image} signifies that the pixel-wise difference between the original and reconstructed images is zero. This visual evidence confirms a \textbf{perfectly lossless reconstruction} for the Manhattan and Chessboard metrics.

In contrast, the Euclidean distance (\texorpdfstring{$L_2$}{L2}) result (Figure 7) is inherently \textbf{lossy} in a discrete domain. Because Euclidean relies on floating-point arithmetic, they do not align perfectly with square pixels, resulting in boundary discrepancies (aliasing) and a non-zero error count (2,462 pixels).
\end{shaded}
