...
def zero_crossings(log_img, search_mask=None, variance_thresh_map=0.0):
...
def run_single_scale_log(norm_image, image, sigma, T_iterative, dynamic_multiplier):
    ...
    return zero_crossings(log_img, search_mask=None, variance_thresh_map=adaptive_thresh_map)

def run_edge_focusing(image):
    mean_val, std_val, T_iterative, dynamic_multiplier = calc_stats(image)
    ...
    for sigma in sigmas_to_process:
        ...
        edges = zero_crossings(log_img, search_mask=current_search_mask, variance_thresh_map=adaptive_thresh_map)
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(dir_pfom, f"{base_name}_single_multi_opencv.pdf"), format='pdf', bbox_inches='tight')
        plt.close()
