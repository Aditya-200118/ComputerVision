...
def generate_opencv_benchmark(image, reference_edges, n_trials=500):
    """
    Final, optimized version. Uses Bayesian Search to find the Canny parameters 
    that most closely align with your Multi-Scale LoG 'Target'.
    """
    # 1. Convert to uint8 for OpenCV
    img_uint8 = np.clip(image, 0, 255).astype(np.uint8)

    def objective(trial):
        # --- Search Space Definition ---
        # Sigma: 0.5 to 4.0 covers everything from sharp detail to heavy noise suppression
        sigma = trial.suggest_float("sigma", 0.5, 4.0)
        
        # High Threshold: The 'seed' for edges. 
        # Low Threshold: The 'link' for edges (must be <= High)
        high_t = trial.suggest_int("high_t", 20, 250)
        low_t = trial.suggest_int("low_t", 5, high_t)
        
        # --- Execution ---
        # Calculate optimal kernel size for the given sigma
        k = int(2 * np.ceil(3 * sigma) + 1)
        if k % 2 == 0: k += 1
        
        blurred = cv2.GaussianBlur(img_uint8, (k, k), sigma)
        
        # L2gradient=True uses the accurate Euclidean distance formula
        candidate = cv2.Canny(blurred, low_t, high_t, L2gradient=True) > 0
        
        # --- Scoring ---
        # We maximize the Pratt's Figure of Merit (PFOM)
        # Higher score = Canny has successfully found the same edges as your LoG
        score = pratt_figure_of_merit(candidate, reference_edges)
        
        return score

    # Create study and suppress logs for clean output
    study = optuna.create_study(direction="maximize")
    optuna.logging.set_verbosity(optuna.logging.WARNING)
    
    print(f"      [Optimization] Finding peak Canny performance...")
    study.optimize(objective, n_trials=n_trials)
    
    # Generate the final 'Perfected' Canny result
    bp = study.best_params
    k_final = int(2 * np.ceil(3 * bp['sigma']) + 1)
    if k_final % 2 == 0: k_final += 1
    
    final_blur = cv2.GaussianBlur(img_uint8, (k_final, k_final), bp['sigma'])
    perfect_canny = cv2.Canny(final_blur, bp['low_t'], bp['high_t'], L2gradient=True)
    
    print(f"      [Result] Sigma: {bp['sigma']:.2f} | High_T: {bp['high_t']} | Low_T: {bp['low_t']} | PFOM: {study.best_value:.4f}")
    return perfect_canny > 0
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        print("  -> Optimizing Canny Parameters for Ground Truth...")
        # We optimize Canny to match our Multi-Scale result as closely as possible
        print("  -> Optimizing Canny Ground Truth via Optuna...")
        # Pass the LoG result so Canny tries to match its quality
        proxy_gt = generate_opencv_benchmark(img, final_multi_edges)
