...
def run_edge_focusing(image):
    ...
    for sigma in sigmas_to_process:
        # print(f"  -> Processing scale \u03C3 = {sigma:.1f}") # Optional console output
...
def plot_and_save_results(image, edge_maps, filename, output_dir):
    ...
    plt.tight_layout()
    out_path = os.path.join(output_dir, f"{filename.split('.')[0]}_edge_focusing_ver5.pdf")
    plt.savefig(out_path, format='pdf', bbox_inches='tight')
    plt.close()

if __name__ == "__main__":
    # Define and create all directories for Version 5
    base_dir = "assignment4_ver5"
    ...
    for file in files:
        ...
        for s in [5.0, 4.0, 3.0, 2.0, 1.0]:
            ...
            # Save raw png for each scale
            plt.imsave(os.path.join(img_single_dir, f"{base_name}_single_scale_{int(s)}.png"), ss_edges, cmap='gray')
            
        # 3. PRATT'S FIGURE OF MERIT & OPENCV BENCHMARK
        print("  -> Generating PFOM vs OpenCV Benchmark...")
        ...
        print(f"     PFOM Single-Scale (\u03C3=1.0): {pfom_single:.4f}")
        ...
        axes[0].axis('off')
        axes[1].imshow(final_multi_edges, cmap='gray')
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(dir_pfom, f"{base_name}_pfom_verification.pdf"), format='pdf', bbox_inches='tight')
        plt.close()

        # 4. QUALITATIVE VISUAL PROOF
        print("  -> Generating Qualitative Visual Proof...")
        fig, axes = plt.subplots(1, 3, figsize=(15, 5))
        axes[0].imshow(single_scale_maps[1.0], cmap='gray')
        axes[0].set_title(f"Single-Scale (\u03C3=1.0)\nHigh Noise / False Positives")
        axes[0].axis('off')
        axes[1].imshow(single_scale_maps[5.0], cmap='gray')
        axes[1].set_title(f"Single-Scale (\u03C3=5.0)\nEdge Drift / Poor Localization")
        axes[1].axis('off')
        axes[2].imshow(final_multi_edges, cmap='gray')
        axes[2].set_title(f"Multi-Scale Focusing (\u03C3=1.0)\nNoise-Free & Localized")
        axes[2].axis('off')
        
        plt.tight_layout()
        plt.savefig(os.path.join(dir_qual, f"{base_name}_qualitative_proof.pdf"), format='pdf', bbox_inches='tight')
        plt.close()
