...
def run_edge_focusing_target(image, target_multiplier):
    """Modified to accept a specific multiplier and return ONLY the final sigma=1.0 map."""
    ...
    for sigma in sigmas_to_process:
        ...
        # Optimization: Test the dynamically injected multiplier against our pipeline stats
...
def generate_opencv_benchmark(image):
    ...
    # 1. State-of-the-art prep: Slight Gaussian Blur
    blurred = cv2.GaussianBlur(img_uint8, (5, 5), 1.0)
    
    # 2. Automated Otsu's thresholding to find optimal Canny bounds
    ...
    # Return as boolean array for mathematical comparison
...
def optimize_pipeline(image, filename):
    print(f"\n[Opt-Pipeline] Starting Grid Search for: {filename}")
    
    # Generate the OpenCV State-of-the-Art benchmark
    print("  -> Generating OpenCV Canny benchmark...")
    canny_baseline = generate_opencv_benchmark(image)
    
    # Define our hyperparameter search grid (from extremely lenient to extremely strict)
    search_space = np.linspace(0.01, 0.15, 15)
    best_multiplier = 0.0
    best_f1 = -1.0
    best_edge_map = None
    ...
    for m in search_space:
        test_edges = run_edge_focusing_target(image, target_multiplier=m)
        f1 = calculate_f1_score(test_edges, canny_baseline)
        
        print(f"     Multiplier: {m:.3f} | F1-Score vs OpenCV: {f1:.4f}")
    ...
    print(f"  [!] Optimal Multiplier Found: {best_multiplier:.3f} (F1: {best_f1:.4f})")
    return best_edge_map, canny_baseline, best_multiplier, best_f1

def plot_optimization_results(image, log_edges, canny_edges, opt_mult, f1_score, filename, output_dir):
    ...
    axes[1].set_title(f"OpenCV Canny (Benchmark)")
    axes[1].axis('off')
    
    axes[2].imshow(log_edges, cmap='gray')
    axes[2].set_title(f"Optimized LoG Focus (m={opt_mult:.3f}, F1={f1_score:.2f})")
    axes[2].axis('off')
    
    plt.tight_layout()
    out_path = os.path.join(output_dir, f"{filename.split('.')[0]}_optimization_compare.pdf")
...
if __name__ == "__main__":
    ...
    for file in files:
        ...
        # Run the self-tuning pipeline
        best_log, cv2_canny, opt_m, final_f1 = optimize_pipeline(img, file)
        
        # Plot the comparison against the State of the Art
        plot_optimization_results(img, best_log, cv2_canny, opt_m, final_f1, file, output_dir)