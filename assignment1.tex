...
\usepackage{mathptmx}       % Math font matching Times
...
\subsection{Feature Extraction Formulation}
For each labeled component, geometric properties were derived using zeroth, first, and second-order moments.

\subsubsection{Area and Centroid}
The area $A$ is defined as the total number of pixels in the component. The centroid $(x_c, y_c)$ is calculated as:
\begin{equation}
    x_c = \frac{1}{A} \sum_{i,j \in R} j, \quad y_c = \frac{1}{A} \sum_{i,j \in R} i
\end{equation}
where $j$ corresponds to the column index (x-coordinate) and $i$ corresponds to the row index (y-coordinate).

\subsubsection{Central Moments}
To determine orientation, coordinates were normalized relative to the centroid:
\begin{equation}
    x' = j - x_c, \quad y' = i - y_c
\end{equation}
The second-order central moments $a$, $b$, and $c$ were computed as:
\begin{align}
    a &= \sum (x')^2 \\
    b &= 2 \sum (x' y') \\
    c &= \sum (y')^2
...
\subsubsection{Orientation and Principal Axes}
The orientation $\theta$ of the axis of elongation (the axis of least inertia) is given by:
\begin{equation}
    \theta = \frac{1}{2} \arctan \left( \frac{b}{a - c} \right)
\end{equation}
The principal moments of inertia, $I_{max}$ and $I_{min}$, representing the major and minor axes variances, are:
\begin{equation}
    I_{max}, I_{min} = \frac{(a + c) \pm \sqrt{(a - c)^2 + b^2}}{2}
\end{equation}

\subsubsection{Eccentricity}
Eccentricity was calculated to classify the shapes (Compact vs. Elongated):
\begin{equation}
...
\begin{figure}[H]
    ...
    \caption{Labeled Components ($Size \ge 100$). \\ Visualization includes Centroids, Bounding Boxes, and Principal Axes}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Labeled Components ($Size \ge 500$). Visualization includes Centroids, Bounding Boxes, and Principal Axes}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Extracted Features for Size $\ge$ 1000. \\Visualization includes Centroids, Bounding Boxes, and Principal Axes}
\end{figure}
...
As the minimum size specification increases, smaller components are progressively suppressed. 
\begin{itemize}
